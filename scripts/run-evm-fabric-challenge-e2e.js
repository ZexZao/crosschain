...
async function main() {
  ...
  try {
    ...
    await timed(stageTimings, 'fabricExecuteHXMsgMs', () =>
      contract.submitTransaction('ExecuteHXMsg', JSON.stringify(hxmsg), hxmsg.callData, JSON.stringify(voucher)));
    ...
    for (const cert of responseVoucher.certifications) {
      if (!(await registry.trustedTEE(cert.teeAddress))) {
        const registerReceipt = await timed(stageTimings, 'evmRegisterResponseTEEMs', async () =>
          (await registry.registerTEE(cert.teeAddress)).wait());
        result.gas.evm.registerTEE += gasOf(registerReceipt);
      }
    }
