...
contract HXMsgGateway {
    ...
    function executeHXMsg(
        ...
        bytes32 deliveryDigest = hxmsg.hashDeliveryFromFull(digest);
        require(cert.requestID == hxmsg.requestID, "cert request mismatch");
        require(cert.hmsgDigest == digest, "cert digest mismatch");
        require(cert.teeAddress == _recover(deliveryDigest, cert.signature), "bad tee signature");
    ...
    function executeHXMsgMinimal(
        ...
        require(cert.requestID == hxmsg.requestID, "cert request mismatch");
        require(cert.hmsgDigest == hxmsg.hmsgDigest, "cert digest mismatch");
        require(cert.teeAddress == _recover(deliveryDigest, cert.signature), "bad tee signature");
        require(teeRegistry.trustedTEE(cert.teeAddress), "untrusted tee");
